...
import { calculate, calculateV4, inverse, inverseV4 } from "./wacca-rate";
...
t.test("WACCA V4 Rate Tests", (t) => {
	function MakeTestCase(score: integer, level: number, expectedRate: number): TestCase {
		return (t) =>
			isAprx(
				t,
				calculateV4(score, level),
				expectedRate,
				`A score of ${score} on a chart of level ${level} should be worth ${expectedRate} rate.`
			);
	}
...
t.test("WACCA Rate Validation Tests", (t) => {
	...
	ThrowsToSnapshot(t, () => calculateV4(-1, 10), "Should throw if score is negative.");
	ThrowsToSnapshot(
		t,
		() => calculateV4(1_000_001, 10),
		"Should throw if score is greater than 1million."
	);
	ThrowsToSnapshot(t, () => calculateV4(900_000, -1), "Should throw if level is negative.");
...
t.test("WACCA V4 Inverse Rate Tests", (t) => {
	function MakeTestCase(expectedScore: integer, level: number, rate: number): TestCase {
		return (t) =>
			isAprx(
				t,
				inverseV4(rate, level),
				expectedScore,
				`A rate of ${rate} on a chart of level ${level} should invert to ${expectedScore} rate.`
			);
	}
...
t.test("WACCA Inverse Rate Validation Tests", (t) => {
	...
	ThrowsToSnapshot(t, () => inverseV4(50, -1), "Should throw if level is negative.");
	ThrowsToSnapshot(t, () => inverse(100, 1), "Should throw if rate is impossible.");
	ThrowsToSnapshot(t, () => inverseV4(100, 1), "Should throw if rate is impossible.");
